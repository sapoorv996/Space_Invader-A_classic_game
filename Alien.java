...
public class Alien {
	...
	public boolean checkShipCollision(int ShipxPos, int ShipyPos) {
		//Is the alien currently alive?
        if (hitState) {
            //If it's alreay been shot then return false;
            return false;
        ...
        //First lets check the X range
        if ((ShipxPos >= leftPosition) && (ShipxPos <= (leftPosition+ALIEN_WIDTH))) {
            //X is ok, now lets check the Y range
            if ((ShipyPos >= heightPosition) && (ShipyPos <= (heightPosition+ALIEN_HEIGHT))) {
                //We shot an alien!
                return true;
            }
        } 
